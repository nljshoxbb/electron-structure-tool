    ...
    persons.forEach(v => {
      ...
      if (v.face) {
        ...
        const info = [
          { label: '性别', value: sex },
          { label: '民族', value: '汉族' },
          { label: '肤色', value: FaceAttrSkincolorMapper[faceAttrObject.skincolor] },
          ...
          { label: '面部表情', value: FaceAttrExpressionMapper[faceAttrObject.faceexpression] },
        ];
      ...
      if (v.body) {
        ...
        Object.keys(attrObject).forEach(i => {
          if (bodyAttrArray.indexOf(i) !== -1) {
            info.push({ label: bodyAttrMapper[i], value: attrObject[i].name || '-' });
          }
        });

        if (attrObject.Category) {
          attrObject.Category.forEach(k => {
            info.push({ label: k.categoryName, value: k.items[0] ? k.items[0].name : '-' });
          });
        }
  ...
  if (vehicles) {
    vehicles.forEach(v => {
      ...
      if (v.attr) {
        ...
        info = info.concat([angleInfo, colorInfo, brandInfo, subbrandInfo, modelyearInfo]);
